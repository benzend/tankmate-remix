		...
		<div>
			...
			{editingName ? (
				...
				<div className="mb-10 flex gap-4 align-baseline">
					<h1 className="cursor-pointer text-center text-base font-bold md:text-lg lg:text-left lg:text-2xl">
						{tank.name}
					</h1>
					<button className="text-slate-300" onClick={handleEditTankNameClick}>
						Edit
					</button>
				</div>
			)}
...
const Count = ({ count, label }: { count: number | null; label: string }) => {
	return (
		<div className={`rounded border-b border-l p-4`}>
			{typeof count === 'number' && (
				<div className="text-7xl italic">{count}</div>
			)}
			{typeof count === 'string' && (
